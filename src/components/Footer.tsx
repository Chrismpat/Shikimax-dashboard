        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          <div className="col-span-1 md:col-span-2">
            ...
            <div className="flex gap-4">
                {[Twitter, Linkedin, Github].map((Icon, i) => (
                    <a key={i} href="#" className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 hover:bg-[#A5D9D9] hover:text-slate-900 transition-all">
                        <Icon size={18} />
                    </a>
                ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-slate-400 font-light text-sm">
                {['Technology', 'Applications', 'Sustainability', 'Partners'].map((item) => (
                    <li key={item}><a href="#" className="hover:text-[#A5D9D9] transition-colors">{item}</a></li>
                ))}
            </ul>
          </div>
          <div>
